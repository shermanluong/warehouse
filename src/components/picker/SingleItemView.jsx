...
const SingleItemView = ({ id }) => {
  ...
  const handlePickPlusQuantity = async (shopifyLineItemId, quantity) => {
    ...
    const pickedQty = currentItem.pickedStatus.verified.quantity +
      currentItem.pickedStatus.damaged.quantity +
      currentItem.pickedStatus.outOfStock.quantity;
                  ...
                  <div className="flex justify-center mt-4 space-x-10">
                    {!lineItems[currentItemIndex].picked && (
                      <div className="relative inline-block">
                        ...
                        {/* DROPDOWN: now BOTTOM of the plus button */}
                        {showDropdown && lineItems[currentItemIndex].quantity > 1 && (
                          <div className="absolute left-1/2 -translate-x-1/2 top-full mt-3 w-28 bg-white border border-gray-300 rounded-xl shadow-xl z-20 flex flex-col items-center">
                            {(() => {
                              const pickedQty = lineItems[currentItemIndex].pickedStatus.verified.quantity +
                                lineItems[currentItemIndex].pickedStatus.damaged.quantity +
                                lineItems[currentItemIndex].pickedStatus.outOfStock.quantity;
                              const remainingQty = lineItems[currentItemIndex].quantity - pickedQty;
                              return Array.from({ length: remainingQty }, (_, i) => i + 1).map((num) => (
                                <button
                                  key={num}
                                  className="w-full py-2 text-lg font-bold text-gray-800 hover:bg-blue-100 rounded-xl border-b last:border-b-0 border-blue-100"
                                  onClick={() => {
